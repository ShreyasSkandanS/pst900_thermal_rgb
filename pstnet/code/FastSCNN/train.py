...
def parse_args():
    ...
    parser.add_argument('--save-folder', default='./weights_size_wft',
...
class Trainer(object):
    def __init__(self, args):
        ...
        input_transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize([.485, .456, .406,0.4], [.229, .224, .225,0.225]),
        ])
    ...
    def validation(self, epoch):
        ...
        for i, (image, target) in enumerate(self.val_loader):
            ...
            avgacc = avgacc+pixAcc
            avgiou = avgiou+mIoU
       	    #pdb.set_trace()
            avgIoU = avgIoU + IoU
        print('Epoch %d, Sample %d, validation pixAcc: %.3f%%, mIoU: %.3f%%' % (
                epoch, i + 1, (avgacc/(i+1)) * 100, (avgiou/(i+1)) * 100))
        print(avgIoU/(i+1))
