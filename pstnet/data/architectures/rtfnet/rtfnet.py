...
class Net(nn.Module):
    ...
    def __init__(self,n_channels, n_class):
        ...
        self.deconvadd = nn.Upsample(size=(45,80),mode='bilinear',align_corners=False)
    ...
    def _make_transpose_layer(self, block, planes, blocks, stride=1,padding=0):

        upsample = None
        if stride != 1:
            upsample = nn.Sequential(
                nn.ConvTranspose2d(self.inplanes, planes, kernel_size=2, stride=stride, padding=padding, bias=False),
    ...
    def forward(self, input):
        ...
        fuse = self.deconvadd(fuse)
        if vobose: print("fuse after deconvadd: ", fuse.size())
